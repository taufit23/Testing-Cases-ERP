---
title: (Manufaktur) — 00. Profil Perusahaan & Master Data
category: accounting-manufaktur
description: Skenario baru berdiri sendiri — CV Sumber Makmur Abadi, manufaktur ringan (keripik kemasan) dengan pinjaman bank, transaksi valas (impor bahan baku), dan periode fiskal kuartalan. Fondasi seluruh file di folder ini, TIDAK terhubung ke suite testing/accounting/ (skenario retail).
visibility: internal
---

# 00. Profil Perusahaan & Master Data

> **Suite ini berdiri SENDIRI, terpisah total dari [`../accounting/`](../accounting/)** (skenario retail "PT Retail Nusantara Jaya") — beda perusahaan, beda kode akun, beda tahun buku, beda tipe periode (kuartalan, bukan bulanan). Jangan campur data/kode akun dari kedua suite.
>
> **Kenapa suite baru, bukan lanjutan suite lama**: suite lama sudah cukup untuk jalur normal (happy path + validasi dasar). Suite ini didesain supaya SETIAP skenario menembus kondisi yang lebih berat & realistis: pinjaman bank+bunga, transaksi mata uang asing sungguhan (bukan cuma probe backend), arus biaya produksi (bahan baku→WIP→barang jadi→HPP), dan periode fiskal kuartalan (variasi dari bulanan). Tiap file punya kolom **Label temuan** (🆕 fitur baru / 🔧 improvement kecil / ✅ sudah tercover) di baris yang hasilnya tidak sesuai ekspektasi ideal.
>
...
**CV Sumber Makmur Abadi** — manufaktur ringan makanan kemasan (keripik singkong & keripik pisang), berlokasi di Cikarang. Sudah **PKP**, PPN 11%. Dua lini produksi dijadikan dimensi Business Unit (bukan dimensi cabang/toko seperti suite lama):

- **Lini Produksi A** — Keripik Singkong
- **Lini Produksi B** — Keripik Pisang

Ciri yang membedakan dari suite lama (sengaja, supaya menguji jalur kode yang belum pernah tersentuh):

1. **Impor bahan baku kemasan/flavoring dari Singapura** (Golden Leaf Trading Pte Ltd) — dibayar USD, transaksi valas SUNGGUHAN lewat `currency_id`/`foreign_debit`/`foreign_credit`/`exchange_rate` di jurnal (bukan cuma dites di level model seperti probe suite lama).
...
| AST-02 | Bank Mandiri Operasional (IDR)       | Aset Lancar | `is_cash_account:true` (saldo awal via `finance/bank-accounts`)                                                             |
| AST-03 | Bank Mandiri Valas (USD)             | Aset Lancar | `is_cash_account:true` (saldo awal via `finance/bank-accounts`, `currency_id:<USD>` — lihat §9, ada temuan penting di sini) |
...
| Kode   | Nama Akun                             | Tipe       |
| ------ | ------------------------------------- | ---------- |
| PDT-01 | Penjualan — Lini A (Keripik Singkong) | Pendapatan |
| PDT-02 | Penjualan — Lini B (Keripik Pisang)   | Pendapatan |
...
| Skenario         | Payload                                                                                | Hasil                                                    |
| ---------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| Positif — Lini A | `{code:"LINI-A", name:"Lini Produksi A - Keripik Singkong", default_unit_type:"both"}` | 201                                                      |
| Positif — Lini B | `{code:"LINI-B", name:"Lini Produksi B - Keripik Pisang", default_unit_type:"both"}`   | 201 — dipakai membedah omzet & biaya per lini di file 04 |
...
| **Catatan keterbatasan (bukan bug)** | PPN Impor / Bea Masuk atas pembelian bahan baku dari Golden Leaf Trading (file 02) TIDAK dimodelkan di suite ini — hanya nilai barang & utang valas yang dites. Kalau mau test PPN Impor sungguhan, itu scope tambahan di luar suite ini | —     |
...
| AST-02 Bank Mandiri Operasional |                       200.000.000 | `finance/bank-accounts`, `currency_id:<IDR>`      | `opening_balance` saja (endpoint ini tidak punya debit/credit_total)                                                                                                                                      |
| AST-03 Bank Mandiri Valas       | 78.500.000 (= USD 5.000 × 15.700) | `finance/bank-accounts`, `currency_id:<USD>`      | `opening_balance:5000` — **lihat bug currency §10.2**                                                                                                                                                     |
